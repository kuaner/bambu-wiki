...
updated: 2026-09-23T02:08:10.018Z
...
![r1_glossary_front-cn.png](https://wiki.bambulab.com/glossary/r1/r1_glossary_front-cn.png)

|  |  |
| --- | --- |
| **1. LED 补光灯** | 位于产品上盖内部，为腔体内提供充足照明，确保俯视摄像头成像清晰。 |
| **2. 激光防护上盖** | 用于隔离激光、保护操作人员安全。上盖配有霍尔传感器，能够检测上盖开合状态，未闭合到位时限制激光启动。 |
| **3. 工具头** | 负责将激光最后反射、聚焦并投射到加工平面，是激光加工的执行端，集成了多种激光、传感与摄像组件。 |
| **4. 支撑刀条** | 用于支撑工件，减少材料与平台的接触面积，避免反射并利于排烟。刀条为可拆卸部件，可以根据材料宽度，合理调整刀条位置。 |
| **5. 俯视摄像头** | 位于产品上盖内部，拍摄整个工作区域的全景图像，结合 AI 算法进行图像拼接与矫正，为后续的定位、识别和其他功能提供精确的视觉基础。 |
| **6. 启停按钮** | 用于发起或停止当前的任务。激光加工任务、校准任务等，都需要按下启停按钮后发起。 |
| **7. 触控屏** | 提供操作与状态显示界面，可通过触控屏进行参数设置、任务控制与信息查看。 |
| **8. 急停开关** | 紧急情况下，按下急停开关可以使机器立即停止运行。正常使用时，请确保其处于释放状态（旋转即可释放）。 |

### 后视图

![r1_glossary_back-cn.png](https://wiki.bambulab.com/glossary/r1/r1_glossary_back-cn.png)

|  |  |
| --- | --- |
| **1. 激光管** | 激光加工的核心部件，负责产生并输出激光能量，经光路系统传输至加工区域。 |
| **2. 电源接口** | 使用电源线将产品连接至市电，打开电源开关后即可开机。 |
| **3. 注水口** | 用于注入冷却液。使用时，需拔出注水口塞子，并插入漏斗再操作。禁止将手指伸入内部，以免受伤。 |
| **4. 水箱视窗** | 可查看当前的水箱水位，视窗旁的温度标识仅用于首次开箱时的水位参考。 |
| **5. 排烟口** | 用于安装排烟风扇，以排出加工产生的烟雾。侧边的 4-pin 接口仅可用于连接排烟风扇。 |
| **6. USB-C 接口** | 可连接 USB-C 数据线（自备），用于数据传输、离线加工或外部设备连接支持。 |
| **7. 安全钥匙** | 已预装安全钥匙。只有插入钥匙，激光相关的功能才能执行。 |
| **8. 4-pin 接口 (Extinguisher/Expansion)** | 用于连接 E1 Pro 净化器或其他拓展配件，两个接口可以混用。 |
| **9. 灭火器气管接口** | 用于连接自动灭火装置的气管，起火时向腔内输送灭火介质。 |

### 工具头

![r1_glossary_toolhead-cn.png](https://wiki.bambulab.com/glossary/r1/r1_glossary_toolhead-cn.png)

|  |  |
| --- | --- |
| **1. 工具头前盖** | 用于保护工具头内部的组件，防止灰尘或杂物进入。 |
| **2. 气嘴** | 位于工具头激光出光口下方，加工时喷出辅助气体，用于吹除加工烟尘、保护镜片，并辅助切割。 |
| **3. 火焰传感器** | 实时监测工具头附近的红外信号与温度，判断加工过程中是否发生起火，提升使用安全性。 |
| **4. 工具头摄像头** | 用于近距离精细拍照，辅助定位、对焦及标记点识别。 |
| **5. 测距激光（蓝十字激光）** | 配合高分辨率相机实现测距，用于自动获取工件表面高度、辅助对焦。 |
| **6. 选点激光（红十字激光）** | 便于用户手动移动工具头时定位出光位置，也用于走边框预览。 |
...
| **3. 挤出机齿轮组件** | 挤出机装置的内部齿轮组件，由一个从动轮和一个主动轮组成，共同作用将耗材送入热端。 |
