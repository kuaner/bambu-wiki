---
path: zh/x2d/maintenance/replace-x-axis-assembly
title: "更换 X 轴组件"
description: ""
tags: []
created: 2026-05-06T14:49:30.853Z
updated: 2026-08-28T06:42:06.250Z
source: https://wiki.bambulab.com/zh/x2d/maintenance/replace-x-axis-assembly
---

## X 轴组件

X 轴组件是工具头载体，工具头安装在X轴上，通过电机的驱动，可实现左右方向的移动。X2D 与 P2S 一样，使用光杆方案。光杆为空心钢棒，由于表面光滑，对碳杆，附着物更容易清理，日常保养更为方便。  
![fac172.jpg](https://wiki.bambulab.com/x2d/maintenance/toolhead-carriage/fac172.jpg)

## 适用场景

- X 轴组件损坏，需要更换

## 所需工具与材料

- 全新的 X 轴组件
- H1.5 内六角扳手
- H2.0 内六角扳手

## 移除 X 轴组件

### 步骤 1. 移除工具头滑车组件

请参考 [更换 X2D 工具头滑车组件](replace-toolhead-carriage.md) 移除工具头滑车组件。  
![remove_the_rear_part.png](https://wiki.bambulab.com/x2d/maintenance/toolhead-carriage/remove_the_rear_part.png)  
![remove_the_front_part.png](https://wiki.bambulab.com/x2d/maintenance/toolhead-carriage/remove_the_front_part.png)

### 步骤 2. 移除 AP 板盖和打印机外壳

1. 请参考 [更换 P2S AP 板盖/LED 补光灯 - 左](../../p2s/maintenance/replace-ap-board-cover-and-led-light-left.md) 移除 AP 板盖：
2. 请参考以下指南拆卸背板和左右侧板：  
   [更换 X2D 背板](replace-rear-panel.md)  
   [更换 X2D 左侧板](replace-left-side-panel.md)  
   [更换 X2D 右侧板](replace-right-side-panel.md)

### 步骤 3. 将 XY 同步皮带从 Y 滑车上移除

用力推动 XY 同步皮带，将同步皮带的固定块顶出一点，然后移除固定块，即可将皮带取出。  
![](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-xy-belt/03_003.png)  
![](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-xy-belt/04_004.png)  
拉动 Y 滑块左右两侧的同步皮带，将同步皮带从 Y 轴滑车上移除。

![09_009.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/09_009.png)  
![11_011.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/11_011.png)

### 步骤 4. 移除 X 轴组件

使用 H1.5 内六角扳手移除 Y 滑车外侧挡板的四颗固定螺丝，将左右两个 Y 滑车的挡板都移除。

![10_010.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/10_010.png)

左右两个 Y 滑块沿 Y 轴相反的方向转动，可将 X 轴与 Y 轴光杆分离，取下 X 轴组件。

![12_012.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/12_012.png)

## 安装 X 轴组件

### 步骤 1. 安装 X 轴组件

将 X 轴组件斜着放入打印机，将 Y 滑车与 Y 直线轴承对准，将 X 轴组件装入。

![12_012.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/12_012.png)

👉 注意：安装完成后，请检查 Y 直线轴承的边缘是否与 Y 滑车对齐。若未对齐，请轻推轴承使与滑车对齐；若轴承突出过多，请将其拆下并重新安装。

![13_013.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/13_013.png)

在装挡板前请先查看挡板上的字母，左：L；右：R。标有 L 的挡板装在打印机的左侧，即靠近左侧板一侧；

![23_023.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/23_023.png)

安装左右两侧的挡板后，使用 H1.5 内六角扳手拧紧 8 颗固定螺丝（每侧 4 颗）。

![10_010.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/10_010.png)

### 步骤 2. 穿 XY 皮带

> 这里以 Y 轴左侧滑块为例进行说明，右侧滑块可以参考，只是上下位置有调换。

首先将皮带末端对折，使皮带有弯折的状态，方便穿过 Y 滑车。

![14_014.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/14_014.png)

参考下图将皮带穿过 Y 滑车的上方惰轮，并从 X 轴光杆伸出。

![15_015.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/15_015.png)

将另一条皮带沿 Y 轴方向穿过 Y 滑车下方的惰轮，并在穿过前方的惰轮后调头，。

![16_016.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/16_016.png)  
![17_017.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/17_017.png)

将调头的皮带再穿过 Y 滑车下方的惰轮，并从 X 轴光杆伸出。

![18_018.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/18_018.png)

将皮带穿入皮带固定座，将固定块的齿与皮带的齿准确对齐，确保凸起的点位于皮带的末端。

![](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-xy-belt/08_008.png)

拉动皮带，将固定快和皮带一起卡入固定座中。

![19_019.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/19_019.png)

![08_008.png](https://public-cdn.bblmw.com/wiki/new/p2s/maintenance/replace-x-axis-assembly-and-toolhead-carriage/08_008.png)

相同的方法将另一侧的皮带穿过 Y 轴滑车，并安装好皮带固定座。

### 步骤 3. 安装 AP 板盖和打印机外壳

1. 请参考以下 WIKI 安装左右侧板和背板：  
   [更换 X2D 左侧板](replace-left-side-panel.md)  
   [更换 X2D 右侧板](replace-right-side-panel.md)  
   [更换 X2D 背板](replace-rear-panel.md)
2. 请参考 [更换 P2S AP 板盖/LED 补光灯 - 左](../../p2s/maintenance/replace-ap-board-cover-and-led-light-left.md) 安装 AP 板盖：

### 步骤 4. 安装工具头滑车组件并张紧 XY 皮带

请参考 [更换 X2D 工具头滑车组件](replace-toolhead-carriage.md) 安装工具头滑车组件，并完成 XY 皮带的张紧。  
![017_front_cover.png](https://public-cdn.bblmw.com/wiki/new/x2d/maintenance/replace_toolhead_cover/017_front_cover.png)  
![tension_screws.png](https://wiki.bambulab.com/x2d/maintenance/toolhead-carriage/tension_screws.png)

## 成功验证

接通电源并启动打印机，运行整机校准流程，确认校准能够顺利完成。

## 结束语

> 我们希望本指南能为您提供有用的信息，帮助您解决问题。  
> 如果本指南未解决您的问题，[请联系在线技术支持（服务时间 9:00-21:00）](https://support.bambulab.cn/liveChat/?from=5)，我们随时准备为您解答疑问并提供帮助。  
> 如果您对本篇 Wiki 有任何建议或反馈，欢迎在评论区留言，感谢您的支持与关注！
