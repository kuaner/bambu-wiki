...
updated: 2026-01-29T08:30:55.265Z
...
PLA与PETG混打是常见的用户场景。我们注意到在此类“低粘接”工况下，料塔稳定性仍有提升空间。针对不易粘接的典型场景，我们对界面层的温度、挤出、熨烫与冲刷流程进行了优化：

- **支持界面层升温增强粘接**：新增参数用于指定界面层升温；其中-1表示升温至该材料的最高打印温度。
- **界面层料塔打印前增加额外挤出增强粘接**：新增界面层预挤出距离与长度参数，界面层在打印料塔前先预挤出一段以增加压力，同时延长挤出距离。
- **优化熨烫策略**：界面层不再执行熨烫，同时支持配置非界面层的熨烫面积参数，保证PETG这类高流动性耗材的熨烫效果。
- **界面层下一层填充降速**：通过降低界面层下一层填充速度，提升界面层基底稳定性，从而增强粘接效果。
- **界面层增加冲刷**：界面层增加冲刷动作，冲刷前擦嘴，冲刷后不再进行擦嘴，直接打印料塔，以减少料头体积并提升界面层高度一致性；新增界面层冲刷长度参数。
- **界面层起始点增加缺口**：在界面层起始处增加缺口，避免因料塔界面层打印前增加的额外挤出导致料头过大引起剐蹭。

经上述策略修改与测试，我们确认该优化能够有效改善PETG作为PLA支撑界面等场景下的料塔倒塌问题。

> ⚠️注意：以上参数均只在开发者模式中可见。

![](https://wiki.bambulab.com//software/bambu-studio/release-note-2-5-0/016.png)
