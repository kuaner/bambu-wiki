...
updated: 2026-09-15T10:46:26.775Z
...
**喷嘴只有在前一层或者热床上挤出耗材时，才能控制挤出宽度**。如果是在空中挤出，由于材料无法被压扁，得到的形状将始终是圆形的（如下图所示），所以无法为桥接、悬空墙设置线宽。具体可以参考：[桥接设置](../../../filament-acc/filament/print-quality/bridging.md)。
...
在Bambu Studio中的**工艺 > 质量 > 线宽**中，可以设置打印各个走线类型的线宽。

![line_width_setting_zh.png](https://wiki.bambulab.com/bambu-studio/parameter/line-width/line_width_setting_zh.png)

### 默认

除首层填充线宽外，其他类型走线的线宽设置为 0 时的默认走线线宽。首层填充线宽设置为0时，将采用首层线宽。

### 首层

设置打印首层的时候的线宽。除首层填充线宽外，若首层线宽设置与其他走线类型的线宽设置冲突，仍然采用设置的首层线宽。首层的默认线宽通常更宽一些，**因为适当增加首层的线宽/层高比可以增强首层的粘附性，确保打印的成功率。**

![initial_layer_line_width_zh.png](https://wiki.bambulab.com/bambu-studio/parameter/line-width/initial_layer_line_width_zh.png)

### 首层填充

用于单独调整首层填充的走线宽度。适当增大该线宽，可以使填充线排列得更加紧密，并增加首层与平台的接触面积，从而改善首层的覆盖效果。  
该设置仅影响首层的稀疏填充、实心填充和顶面，不影响首层墙、支撑等其他部分。设置为 0 时，使用首层线宽。下图蓝色部分则为首层填充线宽应用区域。

![initial_layer_infill_line_width_zh.png](https://wiki.bambulab.com/bambu-studio/parameter/line-width/initial_layer_infill_line_width_zh.png)

### 外墙

设置外墙的走线线宽。模型的最外层壳体属于外墙。（下图中，一号为内墙，二号为外墙）

![outer_wall_line_width_zh.png](https://wiki.bambulab.com/bambu-studio/parameter/line-width/outer_wall_line_width_zh2.png)
...
![top_surface_line_width_zh.png](https://wiki.bambulab.com/bambu-studio/parameter/line-width/top_surface_line_width_zh.png)
