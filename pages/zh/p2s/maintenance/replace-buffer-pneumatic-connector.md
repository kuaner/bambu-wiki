---
path: zh/p2s/maintenance/replace-buffer-pneumatic-connector
title: "更换 P2S 缓冲器气动接头"
description: "本文将详细给您介绍 P2S 缓冲器气动接头更换的步骤和注意事项。"
tags: []
created: 2026-01-05T14:09:38.325Z
updated: 2026-08-28T06:36:07.353Z
source: https://wiki.bambulab.com/zh/p2s/maintenance/replace-buffer-pneumatic-connector
---

## 气动接头（缓冲器）

P2S 缓冲器上使用的气动接头，实际上与 H 系列打印机从动杆上使用的气动接头是一样的，本篇详细介绍了如何更换P2S 缓冲器的气动接头 。
...
> 1. 当前指引要更换的加长版气动接头；
> 2. 普通气动接头；
> 3. AMS 2 Pro 五通组件用到的配件。  
>    ![p_connector.jpg](https://wiki.bambulab.com/p2s/maintenance/replace-pneumatic-connector/p_connector.jpg)

## 何时更换

- 气动接头损坏或脱落

![broken_connector.jpg](https://wiki.bambulab.com/p2s/maintenance/replace-pneumatic-connector/broken_connector.jpg)
...
1. 使用内六角扳手或其他细长工具，插入料筒出口，适度用力撬开料筒。
...
4. 依次装回电路板和黑色上盖，用 H1.5 内六角扳手拧上 4 颗固定螺丝。

![](https://wiki.bambulab.com/p2s/maintenance/buffer-cleaning/21。.png)

### 步骤 3 ：安装缓冲器

请参考[更换 P2S 缓冲器 | Bambu Lab Wiki](replace-filament-buffer.md) 将缓冲器装回打印机，并连接好 AMS 和料管。

![1-connect-ams.jpeg](https://public-cdn.bblmw.com/wiki/new/p2s/manual/first-print/1-connect-ams.jpeg)

## 验证功能

启动打印机，将耗材装载到 AMS 的任一料槽中，然后单击屏幕上的“进料”；进料完成后，再点击“退料”，执行一次退料操作。

![](https://wiki.bambulab.com/p2s/maintenance/buffer-cleaning/9.png)

若进退料过程表现正常，说明缓冲器的气动接头安装成功，可以正常使用。

否则，请再次检查安装后重试。如果未发现异常但仍出现故障，请联系 Bambu Lab 服务团队以获得进一步帮助。
