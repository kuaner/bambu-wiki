---
path: zh/h2/manual/replace-weeding-tool
title: "更换钩针"
description: "本页提供了有关如何更换钩针的说明。"
tags: ["钩针"]
created: 2025-10-11T02:41:23.682Z
updated: 2026-02-11T09:03:52.666Z
source: https://wiki.bambulab.com/zh/h2/manual/replace-weeding-tool
---

## 钩针

钩针可用于提起或移动材料，例如在切割热转印膜等材料后，帮助清除废料。然而，钩针的用途不仅限于清除废料，它还可以协助移除完整的膜。

本页指导您如何更换钩针。

### 步骤 1. 卸下螺丝

卸下钩针主体上的三个螺丝。

![](https://wiki.bambulab.com/h2/maintenance/weeding-tool/remove_screws.png)

### 步骤 2. 打开外壳

如下图所示，轻轻地将钩针推向其主体，慢慢地将外壳的两半分开。小心不要伤到自己哦。

![](https://wiki.bambulab.com/h2/maintenance/weeding-tool/open.png)

### 步骤 3. 移除钩针

打开外壳后，小心拉出钩针，以免造成伤害。

![](https://wiki.bambulab.com/h2/maintenance/weeding-tool/disassemble.png)

![](https://wiki.bambulab.com/h2/maintenance/weeding-tool/tip_removed.png)

### 步骤4.安装新的钩针

安装新的钩针，确保工具上的凹槽与外壳上的凸片对齐。然后，按照步骤2和步骤1的相反顺序重新组装外壳，最后安装螺丝。

![](https://wiki.bambulab.com/h2/maintenance/weeding-tool/final_assembly.png)

## 结束语

> 我们希望所提供的详细指南对您有所帮助并提供信息。
>
> 如果本指南不能解决您的问题，请提交[*技术工单*](https://bambulab.com/zh/my/support/tickets/create?from=5)我们将回答您的问题并提供帮助。
>
> 如果您对本Wiki有任何建议或反馈，请在评论区留言。感谢您的支持与关注！
